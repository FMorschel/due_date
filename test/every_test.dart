...
void main() {
  ...
  group('Skip', () {
    group('SkipCount', () {
      final date = DateTime(2021, DateTime.january, 1);
      const everyMonday = EveryWeekday(Weekday.monday);
      group('Skip once', () {
        const skip = SkipCount(count: 1, every: everyMonday);
        test('Start Date', () {
          final next = DateTime(2021, DateTime.january, 4);
          expect(skip.startDate(date), equals(next));
        });
        test('Next', () {
          final next = DateTime(2021, DateTime.january, 4);
          expect(skip.next(date), equals(next));
        });
        test('Previous', () {
          final previous = DateTime(2020, DateTime.december, 28);
          expect(skip.previous(date), equals(previous));
        });
      });
      group('Skip twice', () {
        const skip = SkipCount(count: 2, every: everyMonday);
        test('Start Date', () {
          final next = DateTime(2021, DateTime.january, 4);
          expect(skip.startDate(date), equals(next));
        });
        test('Next', () {
          final next = DateTime(2021, DateTime.january, 11);
          expect(skip.next(date), equals(next));
        });
        test('Previous', () {
          final previous = DateTime(2020, DateTime.december, 21);
          expect(skip.previous(date), equals(previous));
        });
      });
      group('Skip throw limit exception', () {
        const skip = SkipCount(
          count: 2,
          every: EveryDateValidatorDifference([everyMonday]),
        );
        ...
        test('Start Date', () {
          expect(
            () => skip.startDate(date, limit: date),
            throwsDateTimeLimitReachedException,
          );
        });
        test('Next', () {
          expect(
            () => skip.next(date, limit: date),
            throwsDateTimeLimitReachedException,
          );
        });
        test('Previous', () {
          expect(
            () => skip.previous(date, limit: date),
            throwsDateTimeLimitReachedException,
          );
        });
      });
      group('Skip wont throw limit exception', () {
        final skip = SkipCount(
          count: 2,
          every: const EveryDateValidatorDifference([everyMonday]),
        );
        const oneDay = Duration(days: 1);
        test('Start Date', () {
          final next = DateTime(2021, DateTime.january, 4);
          expect(skip.startDate(date, limit: next.add(oneDay)), equals(next));
        });
        test('Next', () {
          final next = DateTime(2021, DateTime.january, 11);
          expect(skip.next(date, limit: next.add(oneDay)), equals(next));
        });
        test('Previous', () {
          final previous = DateTime(2020, DateTime.december, 21);
          expect(
            skip.previous(date, limit: previous.subtract(oneDay)),
            equals(previous),
          );
        });
      });
    ...
    group('SkipWhen', () {
      final date = DateTime(2021, DateTime.january, 1);
      const everyMonday = EveryWeekday(Weekday.monday);
      group('Skip when day is even', () {
        final skip = SkipWhen(
          when: (date) => date.day.isEven,
          every: everyMonday,
        );
        test('Start Date', () {
          final next = DateTime(2021, DateTime.january, 11);
      ...
      group('Skip when month is January', () {
        final skip = SkipWhen(
          when: (date) => date.month == DateTime.january,
          every: everyMonday,
        );
        test('Start Date', () {
          final next = DateTime(2021, DateTime.february, 1);
          expect(skip.startDate(date), equals(next));
        });
        test('Next', () {
          final next = DateTime(2021, DateTime.february, 1);
    ...
    group('SkipCountWhen', () {
      final date = DateTime(2021, DateTime.january, 1);
      const everyMonday = EveryWeekday(Weekday.monday);
      group('Skip 3 when day is even', () {
        final skip = SkipCountWhen(
          count: 3,
          when: (date) => date.day.isEven,
          every: everyMonday,
        );
        test('Start Date', () {
          final next = DateTime(2021, DateTime.january, 25);
          expect(skip.startDate(date), equals(next));
        });
        test('Next', () {
          final next = DateTime(2021, DateTime.january, 25);
        ...
        test('Previous', () {
          final previous = DateTime(2020, DateTime.december, 7);
          expect(skip.previous(date), equals(previous));
        });
      });
      group('Skip 3 when month is January', () {
        final skip = SkipCountWhen(
          count: 3,
          when: (date) => date.month == DateTime.january,
          every: everyMonday,
        ...
        test('Start Date', () {
          final next = DateTime(2021, DateTime.february, 15);
          expect(skip.startDate(date), equals(next));
        });
        test('Next', () {
          final next = DateTime(2021, DateTime.february, 15);
