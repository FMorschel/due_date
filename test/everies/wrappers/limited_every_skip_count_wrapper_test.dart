...
void main() {
  group('LimitedEverySkipCountWrapper:', () {
    ...
    group('Constructor', () {
      group('Unnamed', () {
        test('Valid basic case', () {
          expect(
            LimitedEverySkipCountWrapper(every: every, count: 1),
            isNotNull,
          );
        });
        test('Can be created as constant', () {
          const wrapper = LimitedEverySkipCountWrapper(
        ...
        test('Creates with correct every', () {
          final wrapper = LimitedEverySkipCountWrapper(every: every, count: 1);
          expect(wrapper.every, equals(every));
        });
        test('Creates with correct count', () {
          final wrapper = LimitedEverySkipCountWrapper(every: every, count: 2);
    ...
    group('Methods', () {
      group('Skip count 0', () {
        final wrapper = LimitedEverySkipCountWrapper(every: every, count: 0);
      ...
      group('Limit handling', () {
        ...
        test('Accepts limit parameter in next', () {
          ...
          final result = wrapper.next(validDate, limit: limit);
        ...
        test('Throws when limit is reached in next', () {
          ...
          expect(
            () => wrapper.next(validDate, limit: limit),
        ...
        test('Accepts limit parameter in previous', () {
          ...
          final result = wrapper.previous(validDate, limit: limit);
          expect(result, isNotNull);
        });
        ...
        test('Throws when limit is reached in previous', () {
          ...
          expect(
            () => wrapper.previous(validDate, limit: limit),
            throwsA(isA<DateTimeLimitReachedException>()),
          );
        });
      ...
      group('processDate', () {
        ...
        test('Uses default count when currentCount is not provided', () {
          ...
          // December 25, 2023 is Monday (skips 2 ahead: December 18 and 25).
          final expected = DateTime(2023, 12, 25);
    ...
    group('Equality', () {
      final wrapper1 = LimitedEverySkipCountWrapper(every: every, count: 1);
      final wrapper2 = LimitedEverySkipCountWrapper(every: every, count: 1);
      final wrapper3 = LimitedEverySkipCountWrapper(every: every, count: 2);
      final wrapper4 = LimitedEverySkipCountWrapper(
